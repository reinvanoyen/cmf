...
function ViewMediaDirectory(props) {
    ...
    const handleLabelFile = (label, fileId) => {
    ...
    const handleChangeFileProperty = (property, value, fileId) => {

        const propertiesMap = ['visibility', 'description', 'copyright'];
    ...
    const handleMoveFile = async (directoryId, fileId) => {
        try {
            await api.media.moveFile(directoryId, fileId);
    ...
    const handleMoveFiles = async (directoryId, fileIds) => {
        try {
            await api.media.moveFiles(directoryId, fileIds);
    ...
    const promptCreateDirectory = () => {
        util.prompt({
            ...
            confirm: value => {
                api.media.createDirectory(value, location.current.params.directory).then(() => {
